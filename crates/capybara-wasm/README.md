...
WebAssembly bindings for `capybara-core` and `capybara-export`, enabling the SQL lineage engine and export formats to run in browsers, Node.js, and other WASM-supported environments.
...
```bash
wasm-pack build crates/capybara-wasm --target web --out-dir ../../packages/core/wasm
```

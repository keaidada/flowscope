...
pub fn analyze_sql(sql_input: &str) -> Result<String, JsValue> {
    // Parse the SQL
    let statements = capybara_core::parse_sql(sql_input)
        .map_err(|e| JsValue::from_str(&format!("Parse error: {e}")))?;

    // Extract table lineage
    let tables = capybara_core::extract_tables(&statements);

    // Create result
    let result = capybara_core::LineageResult::new(tables);
...
pub fn analyze_and_export_sql(request_json: &str) -> Result<String, JsValue> {
    ...
    if result.summary.has_errors {
        let issues: Vec<String> = result
            .issues
            .iter()
            .filter(|i| i.severity == capybara_core::types::Severity::Error)
...
pub fn sanitize_procedure_with_map(sql: &str) -> Option<String> {
    use capybara_core::parser::sanitize_with_line_map;
    use serde::Serialize;
    #[derive(Serialize)]
    struct WithMap {
        dml: String,

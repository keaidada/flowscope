...
use capybara_core::{AnalyzeResult, NodeType, Severity};
...
fn write_lineage(out: &mut String, result: &AnalyzeResult, colored: bool) {
    ...
    for edge in &result.global_lineage.edges {
        if !matches!(
            edge.edge_type,
            capybara_core::EdgeType::DataFlow
                | capybara_core::EdgeType::Derivation
                | capybara_core::EdgeType::JoinDependency
        ) {
            continue;
        }
...
mod tests {
    ...
    fn test_format_table_basic() {
        ...
        assert!(output.contains("Capybara Analysis"));

//! Capybara CLI - SQL lineage analyzer

use capybara_cli::cli;
use capybara_cli::fix::{
    apply_lint_fixes_with_runtime_options, FixCandidateStats, LintFixExecution,
    LintFixRuntimeOptions,
};
use capybara_cli::input;
#[cfg(feature = "metadata-provider")]
use capybara_cli::metadata;
use capybara_cli::output;
use capybara_cli::schema;
#[cfg(feature = "serve")]
use capybara_cli::server;

use anyhow::{Context, Result};
use clap::Parser;
use capybara_core::{analyze, AnalysisOptions, AnalyzeRequest, FileSource, LintConfig, Severity};
use capybara_export::{
...
fn analyze_lint_source(
    ...
    dialect: capybara_core::Dialect,
    lint_config: &LintConfig,
    template_config: Option<capybara_core::TemplateConfig>,
) -> FileLintResult {
        ...
        && has_parse_errors(&result)
    {
        let jinja_retry = analyze(&AnalyzeRequest {
            ...
            schema: None,
            template_config: Some(capybara_core::TemplateConfig {
                mode: capybara_core::TemplateMode::Jinja,
        ...
        if has_template_errors(&jinja_retry) {
            analyze(&AnalyzeRequest {
                ...
                schema: None,
                template_config: Some(capybara_core::TemplateConfig {
                    mode: capybara_core::TemplateMode::Dbt,
                    context: std::collections::HashMap::new(),
                }),
            })
        } else {
...
fn has_parse_errors(result: &capybara_core::AnalyzeResult) -> bool {
    result
        .issues
        .iter()
...
fn has_template_errors(result: &capybara_core::AnalyzeResult) -> bool {
    result
        .issues
        .iter()
...
fn load_schema_metadata(
    args: &Args,
    dialect: capybara_core::Dialect,
) -> Result<Option<capybara_core::SchemaMetadata>> {
...
fn build_request(
    ...
    dialect: capybara_core::Dialect,
    schema: Option<capybara_core::SchemaMetadata>,
    template_config: Option<capybara_core::TemplateConfig>,
) -> AnalyzeRequest {
    if sources.len() == 1 {
        AnalyzeRequest {
...
fn print_issues_to_stderr(result: &capybara_core::AnalyzeResult) {
    use capybara_core::Severity;

...
Command-line interface for the Capybara SQL lineage analyzer.

## Features

- **Multi-File Analysis**: Analyze single or multiple SQL files, or pipe input from stdin.
- **Dialect Support**: Supports all Capybara dialects (PostgreSQL, Snowflake, BigQuery, Generic, etc.).
...
```bash
cargo install --path crates/capybara-cli
```
...
capybara -d snowflake etl/*.sql

# Analyze from stdin
cat query.sql | capybara
...
```bash
capybara -f json -d postgres query.sql
```
...
```bash
capybara -s schema.sql -f mermaid -v column query.sql
```

### CSV Archive Export

```bash
capybara -f csv -o lineage.csv.zip query.sql
```
...
capybara --metadata-url postgres://user:pass@localhost/mydb query.sql

# PostgreSQL: filter to a specific schema
capybara --metadata-url postgres://user:pass@localhost/mydb --metadata-schema analytics query.sql

# MySQL: fetch schema from the connected database
capybara --metadata-url mysql://user:pass@localhost/mydb query.sql

# SQLite: fetch schema from a local database file
capybara --metadata-url sqlite:///path/to/database.db query.sql
```
...
```bash
cargo build -p capybara-cli --no-default-features
```

### dbt and Jinja Templating

Capybara can preprocess SQL files that use Jinja or dbt-style templating before analysis. This enables lineage extraction from dbt models without running dbt itself.

```bash
# Analyze a dbt model with ref() and source() macros
capybara --template dbt models/orders.sql

# Pass variables to templates
capybara --template dbt --template-var target_schema=prod models/*.sql

# Plain Jinja templating (strict mode - undefined variables cause errors)
capybara --template jinja --template-var env=production query.sql
```
...
Capybara can run as a local HTTP server serving the full web UI with a REST API backend. This provides a single-binary deployment where all analysis happens locally.

```bash
# Start server with watched SQL directories
capybara --serve --watch ./sql --watch ./queries

# Use a custom port (default: 3000)
capybara --serve --port 8080 --watch ./models

# Auto-open browser on startup
capybara --serve --watch ./sql --open

# Combine with dialect and database schema
capybara --serve --watch ./sql -d postgres --metadata-url postgres://user@localhost/db
```
...
This command rebuilds `app/dist` and copies the output into `crates/capybara-cli/embedded-app`, which is what gets embedded (and published) with the CLI. Regular `cargo install capybara-cli --features serve` uses these prebuilt assets, so remember to commit the refreshed `embedded-app/` contents when releasing new UI changes.
...
capybara --serve --port 8080 --watch ./sql
```

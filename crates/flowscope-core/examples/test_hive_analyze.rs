...
fn main() {
    ...
    for (i, s) in result.statements.iter().enumerate() {
        ...
        let table_nodes: Vec<_> = s.nodes.iter().filter(|n| n.node_type != flowscope_core::NodeType::Column).collect();
        ...
        let table_edges: Vec<_> = s.edges.iter().filter(|e| table_ids.contains(&e.from) && table_ids.contains(&e.to)).collect();
        println!("  table-level edges: {}", table_edges.len());
        for e in &table_edges {
            let from = table_nodes.iter().find(|n| n.id == e.from).map(|n| n.label.as_ref()).unwrap_or(&e.from);
            let to = table_nodes.iter().find(|n| n.id == e.to).map(|n| n.label.as_ref()).unwrap_or(&e.to);
            println!("    {} -> {}", from, to);
        }
    }
    println!("\nissues:");

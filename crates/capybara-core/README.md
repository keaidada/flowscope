...
Core SQL lineage analysis engine for Capybara.

## Overview

`capybara-core` is a Rust library that performs static analysis on SQL queries to extract table and column-level lineage information. It serves as the foundation for the Capybara ecosystem, powering the WebAssembly bindings and JavaScript packages.
...
```rust
use capybara_core::linter::{Linter, LintConfig, LintDocument};

...
use capybara_core::{analyze, AnalyzeRequest, Dialect, NodeType};
...
fn has_table(result: &capybara_core::AnalyzeResult, table_name: &str) -> bool {
...
#[cfg(feature = "templating")]
use capybara_core::{EdgeType, FileSource};

/// Helper to run multi-file dbt analysis.
#[cfg(feature = "templating")]
fn analyze_dbt_files(files: Vec<FileSource>) -> capybara_core::AnalyzeResult {
    let request = AnalyzeRequest {
        sql: String::new(),
        files: Some(files),
...
#[cfg(feature = "templating")]
fn count_derivation_edges(result: &capybara_core::AnalyzeResult) -> usize {
    result
        .statements
        .iter()

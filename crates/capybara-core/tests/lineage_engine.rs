...
fn column_fk(name: &str, data_type: &str, ref_table: &str, ref_column: &str) -> ColumnSchema {
    use capybara_core::ForeignKeyRef;
    ColumnSchema {
        name: name.to_string(),
        data_type: Some(data_type.to_string()),
        is_primary_key: None,
    ...
    assert!(
        result.issues.is_empty()
            || result
                .issues
                .iter()
                .all(|i| i.severity != capybara_core::Severity::Error),
...
"#;
    ...
    let errors: Vec<_> = result
        .issues
        .iter()
        .filter(|i| i.severity == capybara_core::Severity::Error)
        .collect();
    for e in &errors {
...
fn multiple_join_types_captured() {
    ...
    use capybara_core::JoinType;

    // Helper: find an edge originating from a given node with join_type set
    let find_join_edge = |node_id: &str| -> Option<&capybara_core::Edge> {
        stmt.edges
            .iter()
...
fn cte_join_metadata_captured() {
    ...
    let cte_col_ids: Vec<_> = stmt
        ...
        .iter()
        .filter(|e| {
            e.edge_type == capybara_core::EdgeType::Ownership

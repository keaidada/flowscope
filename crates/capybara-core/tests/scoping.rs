use capybara_core::analyze;
use capybara_core::types::{
...
fn count_warnings<F>(result: &capybara_core::types::AnalyzeResult, predicate: F) -> usize
where
    F: Fn(&capybara_core::types::Issue) -> bool,
{
    result
        .issues
        .iter()
...
fn test_alias_shadowing_in_subquery() {
    ...
    let t1_cols: Vec<_> = edges
        .iter()
        .filter(|e| e.from == *t1_id && e.edge_type == capybara_core::types::EdgeType::Ownership)
        .map(|e| &e.to)
        .collect();
    ...
    let flows_from_t1 = edges.iter().any(|e| {
        ...
        t1_cols.contains(&&e.from) && e.edge_type == capybara_core::types::EdgeType::DataFlow
    });
    ...
    let t2_cols: Vec<_> = edges
        .iter()
        .filter(|e| e.from == *t2_id && e.edge_type == capybara_core::types::EdgeType::Ownership)
        .map(|e| &e.to)
        .collect();

    let flows_from_t2_data = edges.iter().any(|e| {
        t2_cols.contains(&&e.from) && e.edge_type == capybara_core::types::EdgeType::DataFlow
    });
...
fn new_tables_are_known_when_implied_schema_disabled() {
    ...
    let select_tables: Vec<_> = result.statements[1]
        .nodes
        .iter()
        .filter(|n| n.node_type == capybara_core::types::NodeType::Table)
        .collect();
...
fn test_lateral_column_alias_lineage_bigquery() {
    ...
    let col_b = stmt
        .nodes
        .iter()
        .find(|n| n.node_type == capybara_core::types::NodeType::Column && &*n.label == "b");
    let col_c = stmt
        .nodes
        .iter()
        .find(|n| n.node_type == capybara_core::types::NodeType::Column && &*n.label == "c");
    let col_a = stmt
        .nodes
        .iter()
        .find(|n| n.node_type == capybara_core::types::NodeType::Column && &*n.label == "a");
    ...
    let c_derives_from_a = stmt.edges.iter().any(|e| {
        e.from == col_a.id
            && e.to == col_c.id
            && e.edge_type == capybara_core::types::EdgeType::Derivation
    });
...
fn test_lateral_column_alias_lineage_snowflake() {
    ...
    let col_first = stmt
        .nodes
        .iter()
        .find(|n| n.node_type == capybara_core::types::NodeType::Column && &*n.label == "FIRST");
    let col_doubled = stmt
        .nodes
        .iter()
        .find(|n| n.node_type == capybara_core::types::NodeType::Column && &*n.label == "DOUBLED");
    // Source column 'x' from table may be lowercase
    let col_x = stmt.nodes.iter().find(|n| {
        n.node_type == capybara_core::types::NodeType::Column
...
fn test_lateral_column_alias_no_lineage_postgres() {
    ...
    let c_derives_from_a = stmt.edges.iter().any(|e| {
        e.from == col_a.id
            && e.to == col_c.id
            && (e.edge_type == capybara_core::types::EdgeType::Derivation
                || e.edge_type == capybara_core::types::EdgeType::DataFlow)
    });
...
fn test_lateral_column_alias_chain_lineage() {
    ...
    let col_z = stmt
        .nodes
        .iter()
        .find(|n| n.node_type == capybara_core::types::NodeType::Column && &*n.label == "z");

...
export function TableEdge({ id, source, target, data, markerEnd }: EdgeProps) {
  ...
  const srcLayout = useMemo(() => {
    if (!sd) return null;
    const og = (sd as Record<string, unknown>).outputGroups as
  ...
  const tgtLayout = useMemo(() => {
    if (!td) return null;
    const og = (td as Record<string, unknown>).outputGroups as
      | ScriptNodeData['outputGroups']
      | undefined;
  ...
  const curvature = parallelTotal > 1 ? 0.15 + (parallelIndex / (parallelTotal - 1)) * 0.3 : 0.25;
  ...
  const labelYOffset = parallelTotal > 1 ? (parallelIndex - (parallelTotal - 1) / 2) * 14 : 0;
  ...
  return (
    <>
      <BaseEdge
        ...
        path={ep}
        markerEnd={markerEnd}
        style={{ stroke: strokeColor, strokeWidth: strokeW, animation: 'none' }}
      />
      <EdgeLabelRenderer>
        <div
          className="absolute text-[10px] px-1 py-0.5 rounded border"
          style={{

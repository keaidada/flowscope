...
function InsightsScriptNodeComponent({ id, data, selected }: NodeProps): JSX.Element {
  ...
  const { label, sourceName, tableNamesRead, tableNamesWritten, isSelected, isHighlighted } =
    data as ScriptNodeData;
  const outputGroups = (data as Record<string, unknown>).outputGroups as
    | ScriptNodeData['outputGroups']
    | undefined;
  ...
  const groupsKey =
    outputGroups?.map((g) => `${g.inputs.join('|')}:${g.outputs.join('|')}`).join(';') ?? '';
  ...
  const measure = useCallback(() => {
    ...
    for (const rowEl of nodeEl.querySelectorAll<HTMLElement>(
      '[data-read-qname],[data-write-qname]'
    )) {
      ...
      style={{
        ...
        borderColor: active
          ...
          : isHighlighted
            ...
            : s.border,
        boxShadow: active
          ? `0 0 0 2px ${c.interactive.selectionRing}`
          : isHighlighted
            ? `0 0 0 2px ${c.interactive.selectionRing}`
            : undefined,
      }}
    ...
    >
      ...
      <div className="flex items-center gap-2 px-3 py-2.5">
        <div
          ...
          style={{ backgroundColor: s.headerBg, color: s.accent }}
        >
          <FileCode className="h-4 w-4" strokeWidth={1.5} />
        </div>
        <div className="flex-1 overflow-hidden">
          <div className="flex items-center gap-1">
            <div className="truncate text-sm font-semibold" style={{ color: s.text }} title={label}>
              {label}
            </div>
            <button
              onClick={handleCopy}
            ...
            >
              {copied ? (
                <Check className="h-3 w-3" style={{ color: c.status.success }} />
              ) : (
                <Copy className="h-3 w-3" style={{ color: s.textSecondary }} />
              )}
            </button>
          </div>
          <div
          ...
          >
            <span>入 {reads.length}</span>
            <span>出 {writes.length}</span>
          </div>
        </div>
        <button
        ...
        >
          {copiedAll ? (
            <Check className="h-3.5 w-3.5" style={{ color: c.status.success }} />
          ) : (
            <ClipboardList className="h-3.5 w-3.5" style={{ color: s.textSecondary }} />
          )}
        </button>
        <button
          onClick={toggle}
        ...
        >
          {expanded ? (
            <ChevronUp className="h-4 w-4" style={{ color: s.textSecondary }} />
          ) : (
            <ChevronDown className="h-4 w-4" style={{ color: s.textSecondary }} />
          )}
        </button>
      </div>
      ...
      {expanded && (
        <div className="border-t" style={{ borderColor: s.border }}>
          {isGrouped ? (
            <GroupedTables groups={layout.groups} sourceName={sourceName} c={c} s={s} />
          ) : (
            <FlatTables reads={reads} writes={writes} sourceName={sourceName} c={c} s={s} />
          )}
          {reads.length === 0 && writes.length === 0 && (
            <div className="px-3 py-2 text-xs italic" style={{ color: s.textSecondary }}>
              无表依赖
            </div>
          )}
        </div>
    ...
    <>
      {reads.length > 0 && (
        <div className="px-2 py-1.5">
          <div className="text-xs font-semibold mb-1 px-1" style={{ color: c.status.success }}>
            输入 ({reads.length})
          </div>
          {reads.map((t) => {
            ...
            return (
              <div
                key={t}
                data-read-qname={t}
                className="text-xs flex items-center cursor-pointer rounded transition-colors"
                style={{
                  color: isHL ? c.interactive.selection : s.textSecondary,
                  backgroundColor: isHL ? c.interactive.hover : 'transparent',
                  fontWeight: isHL ? 600 : 400,
                  padding: '3px 6px',
                }}
                onClick={(e) => {
                  e.stopPropagation();
                  toggleHighlight(t, 'read', sourceName);
                }}
                title={t}
              >
                <span
                  className="w-1.5 h-1.5 rounded-full shrink-0 mr-1.5"
                  style={{ backgroundColor: c.status.success }}
                />
                <span className="break-all flex-1">{t}</span>
                {isHL && (
                  <span
                    className="text-[10px] font-semibold px-1 rounded shrink-0 ml-1"
                ...
                <button
                  ...
                  className="shrink-0 p-0.5 rounded hover:bg-black/10 dark:hover:bg-white/10 ml-1"
                  title="复制"
                >
                  <Copy className="h-3 w-3" style={{ color: s.textSecondary }} />
                </button>
      ...
      {writes.length > 0 && (
        <div className="px-2 py-1.5 border-t" style={{ borderColor: `${s.border}44` }}>
          <div className="text-xs font-semibold mb-1 px-1" style={{ color: c.status.info }}>
            输出 ({writes.length})
          </div>
          {writes.map((t) => {
            ...
            return (
              <div
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  toggleHighlight(t, 'write', sourceName);
                }}
                title={t}
              >
                <span
                  className="w-1.5 h-1.5 rounded-full shrink-0 mr-1.5"
                  style={{ backgroundColor: c.status.info }}
                />
                <span className="break-all flex-1">{t}</span>
                {isHL && (
                  <span
...
interface GroupedTablesProps {
  groups: Array<{
    inputs: Array<{ qname: string; y: number }>;
    outputs: Array<{ qname: string; y: number }>;
    startY: number;
  }>;
  sourceName: string;
  c: ReturnType<typeof useColors>;
...
function GroupedTables({ groups, sourceName, c, s }: GroupedTablesProps) {
  return (
    <>
      {groups.map((group, gi) => (
        <div key={gi}>
          {gi > 0 && <div className="border-t my-0.5" style={{ borderColor: `${s.border}88` }} />}
          {group.inputs.length > 0 && (
            <div className="px-2 py-1.5">
              <div className="text-xs font-semibold mb-1 px-1" style={{ color: c.status.success }}>
                输入 ({group.inputs.length})
              </div>

  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="flex items-center justify-between text-[11px] text-muted-foreground">
        <span>{t('globalLineageList.upstreamMini', { count: upstreamCount })}</span>
        <span>{t('globalLineageList.downstreamMini', { count: downstreamCount })}</span>
      </div>
    </div>
  );
}
...
function DrawerStatCard({
  ...
  return (
    <div className="min-w-[132px] flex-1 rounded-xl border border-border bg-muted/20 px-4 py-3">
      ...
      <div className={cn('mt-1 text-xl font-semibold text-foreground', accentClassName)}>
        {value}
      </div>
    </div>
  );
...
export function GlobalLineageListView({
  ...
  const [expandedInventoryGroups, setExpandedInventoryGroups] = useState<Set<string>>(
    () => new Set()
  );
  ...
  const quickViewCounts = useMemo(
    () => ({
      ...
      hotspots: Math.min(
        graphData.entries.length,
        Math.max(8, Math.ceil(graphData.entries.length * 0.2))
      ),
  ...
  const openEntry = useCallback(
    (nodeId: string, context?: SelectionContext) => {
  ...
  const handlePreviousStep = useCallback((step: number) => {
  ...
  const handleReturnToSource = useCallback(() => {
    ...
    if (selectionContext.file) {
      ...
      setExpandedInventoryGroups(
        (previous) => new Set([...Array.from(previous), selectionContext.file!])
      );
    }
  }, [selectionContext]);
  ...
  const handleSelectRelatedTable = useCallback(
    (nodeId: string) => {
      openEntry(
        nodeId,
        selectedEntry ? { kind: 'relation', label: selectedEntry.qualifiedName } : undefined
      );
    },
      ...
      <section className="flex min-h-screen min-h-0 flex-col rounded-2xl border border-border bg-background">
        ...
        <div className="min-h-0 flex-1 overflow-hidden">
          {totalFilteredEntries === 0 ? (
            ...
            <div ref={tableScrollRef} className="max-h-[calc(100vh-260px)] overflow-auto">
          ...
          <div className="sticky bottom-0 z-20 shrink-0 border-t border-border bg-background/95 px-3 py-2.5 backdrop-blur supports-[backdrop-filter]:bg-background/80">
            <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="flex items-center gap-1.5 rounded-md border border-border bg-background px-2 py-1">
                  <span className="text-xs text-muted-foreground">
                    {t('globalLineageList.jumpToPage')}
                  </span>
                  <input
                    type="number"
                    min={1}

...
if [[ "$OS_NAME" == "macOS" ]]; then
    case "$ARCH" in
        arm64)  TARBALL="$SCRIPT_DIR/capybara-2.0.0-darwin-arm64.tar.gz" ;;
        x86_64) TARBALL="$SCRIPT_DIR/capybara-2.0.0-darwin-x86_64.tar.gz" ;;
    esac
fi
...
if command -v lsof &>/dev/null; then
    while lsof -i:"$PORT" &>/dev/null; do
        warn "端口 $PORT 已被占用，自动切换到 $((PORT + 1))"
        PORT=$((PORT + 1))
    done
fi
info "端口: $PORT ✅"
